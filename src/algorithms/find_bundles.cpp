...
pair<bool, Bundle*> is_in_bundle(const handle_t& handle, const HandleGraph& g,
    ...
    bool has_reversed = false; // TODO: Is keeping track of reversed necessary?
    ...
    unordered_set<handle_t> lhs_new; /// All handles that aren't part of a balanced bundle set
    bool is_first = true;
    ...
    /// If the bundle is unbalanced, then continue looking for both sides
    ...
    return pair(true, bundle);
}

pair<bool, Bundle*> find_bundle(const handle_t& handle, const HandleGraph& g) {
    unordered_set<handle_t> cache;
    auto [is_bundle, bundle] = is_in_bundle(handle, g, cache);
    return pair(is_bundle, bundle);
}

vector<Bundle*> find_bundles(const HandleGraph& g) {
