...
std::pair<bool, Bundle> is_in_bundle(const handle_t& handle, traversed_t& traversed_nodes, const HandleGraph* g, bool go_left = false) {
    Bundle bundle;
    // Get which side of the bundle the handle is on
    bool handle_dir     = g->get_is_reverse(handle);
    bool is_handle_left = (handle_dir && go_left) || (!handle_dir && !go_left);

    // Insert edges from opposite side
    g->follow_edges(handle, !is_handle_left, [&](const handle_t& opp_handle) {
    ...
    for (const handle_t& opp_handle : opposite_side) {
        bool not_bundle = g->follow_edges(opp_handle, is_handle_left, [&](const handle_t& same_handle) -> bool {
